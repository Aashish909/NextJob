...
const JobsPageContent = () => {
  ...
  useEffect(() => {
    if (searchQuery) {
      ...
      fetch(`/api/job/search?q=${encodeURIComponent(searchQuery)}`)
        .then((res) => res.json())
        .then((data) => {
  ...
  return (
    <div>
      <div className="w-[80%] m-auto mt-3">
          ...
          searchLoading ? (
            ...
            <div className="flex items-center flex-wrap gap-9 md:gap-10 p-2">
              {searchJobs && searchJobs.length > 0 ? (
                searchJobs.slice((page - 1) * 6, page * 6).map((e) => {
        ...
        {(searchQuery ? searchJobs && searchJobs.length > 6 : jobs && jobs.length > 6) && (
